...
#load_dotenv()
openai.api_key = "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"
...
Always remember the URL to be used:"\"https://www.farmers.com/"
---
...
def get_existing_function_count():
    if not Path("son_test_farmer.py").exists():
        return 0
    with open("json_test_farmer.py") as f:
        return len(re.findall(r"^def f\d+\(", f.read(), re.M))

def ensure_script_header():
    file_path = "json_test_farmer.py"
...
def regenerate_run_all_tests(upto: int):
    ...
    with open("json_test_farmer.py", "r") as f:
        content = f.read()
    ...
    run_all_tests += "        page.goto(\"https://www.farmers.com/\")\n\n"
    ...
    with open("json_test_farmer.py", "w") as f:
        f.write(content)
...
def remove_style_script(html):
    ...
    # Remove <form>...</form> (optional, if forms are not needed)
    html = re.sub(r'<form.*?>.*?</form>', '', html, flags=re.DOTALL | re.IGNORECASE)

    # Remove <header>, <footer>, <nav>
    html = re.sub(r'</?(header|footer|nav)[^>]*>', '', html, flags=re.IGNORECASE)
    ...
    # Remove inline styles, classes, ids, data-* attributes, JS handlers
    html = re.sub(r'\s*(style|class|id|data-[\w-]+|onclick|onmouseover|onload)\s*=\s*(".*?"|\'.*?\')', '', html, flags=re.IGNORECASE)
...
def main():
    # Delete existing /home/srikarikatrapati/Documents/brizo_framers/json_test_farmer.py to start fresh
    if Path("json_test_farmer.py").exists():
        try:
            Path("json_test_farmer.py").unlink()
            print("Removed existing /home/srikarikatrapati/Documents/brizo_framers/json_test_farmer.py to start fresh")
        except Exception as e:
            print(f"Warning: Could not remove existing /home/srikarikatrapati/Documents/brizo_framers/json_test_farmer.py: {e}")
    ...
    with open("tests_farmers.json") as f:
        plan = json.load(f)
    ...
    for i, node in enumerate(plan["nodes"]):
        ...
        try:
            ...
            subprocess.run(["python", "json_test_farmer.py", str(idx)], timeout=300)
            #subprocess.run(["C:/Users/HP/Desktop/Brizo/venvv/Scripts/python.exe", "/home/srikarikatrapati/Documents/brizo_framers/json_test_farmer.py", str(idx)], timeout=100)
